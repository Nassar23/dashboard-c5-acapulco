...
# Configuración de la página - responsive para móviles, tablets y PC
st.set_page_config(
    page_title="Monitor de Cámaras C5",
    page_icon="📹",
    layout="wide",
    initial_sidebar_state="auto"
)

# CSS para mejorar la visualización en móviles y tablets
st.markdown("""
<style>
    /* Ajustes responsivos */
    ...
    /* Métricas más legibles en móviles */
...
def descargar_archivo_drive(file_id):
    """Descargar archivo desde Google Drive a memoria"""
    service = get_drive_service()
...
def cargar_catalogo_fallas():
    """Carga el catálogo de fallas desde el primer archivo de Drive"""
    ...
    try:
        ...
        st.sidebar.warning(f"No se pudo cargar catálogo: {str(e)}")
...
@st.cache_data(ttl=300)
def cargar_datos():
    """Cargar datos desde Google Drive procesando todos los archivos"""
    archivos = listar_archivos_drive()
    ...
    for archivo in archivos:
        ...
        # Saltar archivos duplicados (con paréntesis en el nombre)
        ...
        try:
            ...
            archivos_info.append({'nombre': nombre_sin_ext, 'fecha': fecha_str, 'registros': len(df)})
...
def clasificar_estatus(estatus):
    """Clasificar el estatus en OPERATIVA, CON FALLA u OTRO"""
...
if df_completo is None or len(df_completo) == 0:
    ...
    st.info("Verifica que existan archivos Excel con el formato: C5_Acapulco_DDMMAA.xlsx en la carpeta configurada")
    st.stop()

if 'ID CÁMARA' not in df_completo.columns:
    st.error("❌ Falta columna ID CÁMARA en los archivos")
    st.stop()
...
with st.sidebar:
    st.success(f"✅ {len(archivos_info)} archivos")
...
# Usar 2 columnas en móviles, 5 en desktop con CSS
...
col2.metric("📋 Barridos", total_barridos)
...
col5.metric("📊 Tasa de Fallas", f"{(camaras_con_fallas/total_camaras*100):.1f}%" if total_camaras > 0 else "0%")
...
with tab1:
    ...
    with col_b:
        ...
        fig2.update_layout(showlegend=False, height=350, margin=dict(l=10, r=10, t=30, b=10),
                          xaxis_title="Sistema", yaxis_title="Cantidad")
...
with tab2:
    ...
    if len(df_fallas_dia) > 0:
        ...
        fig_fallas.update_layout(xaxis_tickangle=-45, height=500, margin=dict(l=10, r=10, t=50, b=10))
        ...
        with col_c:
            ...
            fig_tipo.update_layout(height=400, margin=dict(l=10, r=10, t=50, b=10))
...
with tab3:
    ...
    if len(df_fallas_rec) > 0:
        ...
        with col_gantt1:
            max_dias = int(fallas_por_camara['Días con Falla'].max())
        ...
        with col_tabla1:
            umbral_tabla = st.slider(
                "Filtrar por días de falla:",
                1,
...
with tab4:
    ...
    with col_e:
        ...
        fig_zonas.update_layout(height=400, margin=dict(l=10, r=10, t=30, b=10))
    ...
    with col_f:
        ...
        if len(df_fallas_zona) > 0:
            ...
            fig_fz.update_layout(height=400, margin=dict(l=10, r=10, t=30, b=10))
    ...
    zona_sel = st.selectbox(
        "Filtrar por zona:",
        ['Todas'] + sorted(df_ultimo['ZONA'].unique().tolist()),
        key="zona_tab4"
    )
...
st.caption(
    ...
    f"📊 {len(df_completo)} registros | "
    f"📁 {len(archivos_info)} archivos | "
    f"📅 Periodo: {df_completo['fecha_str'].min()} - {df_completo['fecha_str'].max()}"
)
